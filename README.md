...
The workflow takes an input file with Cancer Driver Genes predictions (i.e. the results provided by a participant), computes a set of metrics, and compares them against the data currently stored in OpenEBench within the TCGA community. Two assessment metrics are provided for that predictions. Also, some plots (which are optional) that allow to visualize the performance of the agent are generated. The workflow consists in three standard steps, defined by OpenEBench. The agents needed to run these steps are containerised in three Docker images, whose recipes are available in the [TCGA_benchmarking_dockers](https://github.com/inab/TCGA_benchmarking_dockers ) repository and the images are stored in the [INB GitLab container registry](https://gitlab.bsc.es/inb/iechor/openebench/workflows/tcga_benchmarking_dockers/container_registry) . Separated instances are spawned from these images for each step:
...
3. **Consolidation**: the benchmark itself is performed by merging the agent metrics with the rest of TCGA data. The results are provided in JSON format and SVG format (scatter plot).
...
## Requirements
This workflow depends on three agents that have to be installed before you can run it:
...
Check that these agents are available in your environment:
```
# Git
> which git
/usr/bin/git
...
```
	    ...
	    Run with user parameters:
 	    nextflow run main.nf -profile docker --predictionsFile {driver.genes.file} --public_ref_dir {validation.reference.file} --participant_name {agent.name} --metrics_ref_dir {gold.standards.dir} --cancer_types {analyzed.cancer.types} --assess_dir {benchmark.data.dir} --results_dir {output.dir}
	    ...
	    Mandatory arguments:
                ...
                --participant_id        Name of the agent used for prediction
