...
if (params.help) {
	    ...
	    Run with user parameters:
 	    ...
 	    nextflow run main.nf -profile docker --input {driver.genes.file} --public_ref_dir {validation.reference.file} --participant_id {agent.name} --goldstandard_dir {gold.standards.dir} --cancer_types {analyzed.cancer.types} --assess_dir {benchmark.data.dir} --augmented_assess_dir {benchmark.augmented_data.dir} --results_dir {output.dir}
	...
	log.info """\
         ...
         agent name : ${params.participant_id}
...
agent_name = params.participant_id.replaceAll("\\s","_")
...
process validation {
	...
	"""
	python /app/validation.py -i $input_file -r $ref_dir -com $community_id -c $cancer_types -p $agent_name -o validation.json
	"""
...
process compute_metrics {
	...
	"""
	python /app/compute_metrics.py -i $input_file -c $cancer_types -m $gold_standards_dir -p $agent_name -com $community_id -o assessment.json
	"""
